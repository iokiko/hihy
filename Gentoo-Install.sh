...
start_pre() {
    checkpath --directory --owner root:root --mode 0755 /run/hihy
}
...
start() {
    ebegin "Starting hihyServer "
    start-stop-daemon --start --background --exec /etc/hihy/bin/appS -- \
    --log-level info -c /etc/hihy/conf/hihyServer.json server 
    eend $?
}

stop() {
    ebegin "Stopping hihyServer"
